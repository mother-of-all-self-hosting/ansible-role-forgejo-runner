...
#
# Tags look like `v<Forgejo Runner version>-<release>`:
#
# - if defaults/main.yml points at a Forgejo Runner version that has never been
#   released, the release counter restarts at 0 (`v12.13.1-0`)
# - otherwise the counter is incremented (`v12.13.1-1`), but only if something
...
version="$(sed -nE 's|^forgejo_runner_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Forgejo Runner version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `12.13.0`), but the tags
# do (`v12.13.0-0`). Stripping any `v` before prepending one keeps this correct
